...
interface CartButtonProps {
    ...
    color?: string;
    quantity?: number;
    disabled?: boolean; // optional if button should be disabled externally
}
...
const CartButton: React.FC<CartButtonProps> = ({
    ...
    const handleAddToCart = () => {
        if (!size || !color) {
            setPopupText("Please select size and color");
            setShowPopup(true);
            setTimeout(() => setShowPopup(false), 2000);
            return;
        }
    ...
    return (
        <div className="relative">
            <button
                ...
                className={`w-full px-6 py-3 rounded-md font-semibold transition-colors ${disabled || !size || !color
